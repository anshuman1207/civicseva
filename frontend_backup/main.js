  ...
  if (selector) {
    selector.addEventListener('change', (e) => {
      i18next.changeLanguage(e.target.value).then(() => {
        ...
        // Dynamically change font family
        ...
        setMapLanguage(e.target.value); // Dynamically update Google Map baseline tiles
...
async function loadUserStats(token) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      // Animate Gamification Points counting up!
      const currentPts = parseInt(pointsEl.innerText) || 0;
      animate(currentPts, user.points, {
        duration: 1.5,
        ease: "easeOut",
        onUpdate: (latest) => {
          pointsEl.innerText = Math.round(latest);
        }
      });

      // Update level text from DB
      levelEl.innerText = user.level || 'Beginner';
    } else {
      // Backend returned error — show defaults  
      levelEl.innerText = 'Beginner';
      pointsEl.innerText = '0';
    }
  } catch(err) {
    // Backend unreachable (placeholder URL) — show defaults instead of "Loading..."
    console.warn('Backend offline, showing defaults:', err.message);
    levelEl.innerText = 'Beginner';
    pointsEl.innerText = '0';
  }
}

// Immediately clear "Loading..." with defaults, then try to fetch real stats
loadUserStats(null);
ensureAuth().then(token => { if (token) loadUserStats(token); });

let currentTileLayer = null;
function setMapLanguage(lang) {
  if (currentTileLayer) map.removeLayer(currentTileLayer);
  
  // Use Google Maps with dynamic language parameter (hl)
  currentTileLayer = L.tileLayer(`https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}&hl=${lang}`, {
...
function initMap() {
  ...
  // Add Dynamic Google Maps Tiles
  setMapLanguage(i18next.language || 'en');
